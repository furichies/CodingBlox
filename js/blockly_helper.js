...
*/
function saveCode() {
  var fileName = window.prompt('¿Qué nombre quieres para el fichero?', 'Kubot')
...
function discard() {
  var count = Blockly.mainWorkspace.getAllBlocks().length;
  if (count < 2 || window.confirm('¿Borramos los ' + count + ' bloques?')) {
    Blockly.mainWorkspace.clear();
    renderContent();
  }
}
...
function onSuccess() {
  if (ajax.readyState == 4) {
    if (ajax.status == 200) {
      ...
      //MODIFICADO PARA QUE LIMPIE EL WORKSPACE SIN PREGUNTAR CUANDO CARGA UN JUEGO.
      //var count = Blockly.mainWorkspace.getAllBlocks().length;
      //if (count && confirm('¿Reemplazar los bloques existentes?\n"Cancel" mezclará.')) {
        Blockly.mainWorkspace.clear();
      //}
...
function uploadCode(code, callback) {
    ...
    request.onreadystatechange = function() {
        ...
        case 200:
            break;
        case 0:
            errorInfo = "code 0\n\nNo es posible conectar con el servidor " + url + ".  ¿Se está ejecutando el servidor?";
            break;
        case 400:
            errorInfo = "code 400\n\nError de compilación.  Algo anda mal con los bloques.";
            break;
        case 500:
            errorInfo = "code 500\n\nError en la subida.  ¿Conectaste la placa al USB?";
            break;
        case 501:
...
function uploadClick() {
    ...
    alert("Preparados para actualizar Arduino");
    
    uploadCode(code, function(status, errorInfo) {
        if (status == 200) {
            alert("Actualización de programa ok");
        } else {
            alert("Error actaulizando programa: " + errorInfo);
        }
    });
}
